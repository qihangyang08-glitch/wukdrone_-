...
**标准编译 (修改代码后执行)**
```bash
cd /home/qwssx2/ardupilot && make sitl
./waf copter
```
...
sim_vehicle.py -v ArduCopter --console --map -w --add-param-file=Tools/autotest/default_params/wuk.parm
```

**极速启动 (仅控制台)**
*适合快速测试代码逻辑，省资源。*
```bash
sim_vehicle.py -v ArduCopter --console -w --add-param-file=Tools/autotest/default_params/wuk.parm
```
...
```bash
cd /home/qwssx2/ardupilot && g++ tools/wuk_log_parser.cpp -o tools/wuk_log_parser
```

**运行解析 (Parse Log)**
*SITL 的最新日志通常在 `logs/LASTLOG.BIN` 或当前目录下的 `eeprom.bin` 同级目录。*
```bash
cd /home/qwssx2/ardupilot && ./tools/wuk_log_parser wuk_gb.log
```
*产出：`wuk_parsed.csv`, `wuk_summary.txt`*

**生成图表 (Generate Plots)**
```bash
cd /home/qwssx2/ardupilot && python3 tools/wuk_plot.py wuk_parsed.csv
```
